...
export function ExerciseVisual({ name }: { name: string }) {
  return (
    <div className="grid gap-4 md:grid-cols-[1.1fr_0.9fr]">
      <div className="relative min-h-[280px] overflow-hidden rounded-lg border border-line bg-gradient-to-br from-panel to-black p-6">
        <div className="absolute right-6 top-6 rounded-full border border-blood/40 px-3 py-1 text-xs font-bold uppercase text-ember">motion guide</div>
        <div className="mx-auto mt-12 grid h-48 w-48 place-items-center rounded-full border border-blood/40 bg-blood/5">
        ...
        <p className="mt-5 text-center text-sm font-bold text-zinc-300">{name} position path</p>
      </div>
        ...
        {[
          ...
          <div key={String(title)} className="rounded-lg border border-line bg-panel p-4">
            <div className="flex items-start gap-3">
              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-md bg-blood/15 text-ember">
